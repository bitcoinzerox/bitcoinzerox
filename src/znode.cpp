...
bool CZnodeBroadcast::CheckOutpoint(int &nDos) {
...

    {
        ...
        if (coins.vout[vin.prevout.n].nValue != ZNODE_COIN_REQUIRED * COIN) {
            LogPrint("znode", "CZnodeBroadcast::CheckOutpoint -- Znode UTXO should have 1000 HXX, znode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 1000 HXX tx got nZnodeMinimumConfirmations
