...
//        LogPrintf("mnpayments.mapXnodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2)=%s\n", mnpayments.mapXnodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2));
        if (mnpayments.mapXnodeBlocks.count(BlockReading->nHeight) &&
            ...
            CAmount nXnodePayment = GetXnodePayment(BlockReading->nHeight, block.vtx[0].GetValueOut());
...
bool CXnodeBroadcast::CheckOutpoint(int &nDos) {
...

    {
        ...
        if (coins.vout[vin.prevout.n].nValue != XNODE_COIN_REQUIRED * COIN) {
            LogPrint("xnode", "CXnodeBroadcast::CheckOutpoint -- Xnode UTXO should have 2000 HXX, xnode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 2000 HXX tx got nXnodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex *pMNIndex = (*mi).second; // block for 2000 HXX tx -> 1 confirmation
